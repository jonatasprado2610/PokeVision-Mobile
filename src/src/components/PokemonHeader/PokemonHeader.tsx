import { IonSearchbar } from '@ionic/react';
import {
  FaFire, FaTint, FaLeaf, FaBolt, FaSnowflake,
  FaFistRaised, FaSkull, FaMountain, FaFeatherAlt,
  FaBrain, FaDragon, FaCircle
} from 'react-icons/fa';
import { GiSpartanHelmet } from 'react-icons/gi';
import './PokemonHeader.scss';

const GENS = ['I','II','III'];

const TYPES = [
  { name: 'Normal',   color: '#A8A878', text: '#3a3a1a', icon: <FaCircle /> },
  { name: 'Fogo',     color: '#FF603F', text: '#5a0e00', icon: <FaFire /> },
  { name: 'Água',     color: '#76BDFE', text: '#002f5a', icon: <FaTint /> },
  { name: 'Planta',   color: '#78C850', text: '#1a4a00', icon: <FaLeaf /> },
  { name: 'Elétrico', color: '#F8D030', text: '#4a3a00', icon: <FaBolt /> },
  { name: 'Gelo',     color: '#98D8D8', text: '#003a3a', icon: <FaSnowflake /> },
  { name: 'Luta',     color: '#C03028', text: '#ffe0df', icon: <FaFistRaised /> },
  { name: 'Veneno',   color: '#A890F0', text: '#1a0066', icon: <FaSkull /> },
  { name: 'Terra',    color: '#E0C068', text: '#3a2800', icon: <FaMountain /> },
  { name: 'Voador',   color: '#90C0F8', text: '#00225a', icon: <FaFeatherAlt /> },
  { name: 'Psíquico', color: '#F85888', text: '#5a0020', icon: <FaBrain /> },
  { name: 'Dragão',   color: '#7038F8', text: '#f0e0ff', icon: <FaDragon /> },
];

interface Props {
  onSearch: (q: string) => void;
  onGenChange: (gen: string) => void;
  onTypeToggle: (type: string) => void;
  selectedGen: string;
  selectedTypes: string[];
}

const PokemonHeader: React.FC<Props> = ({
  onSearch, onGenChange, onTypeToggle, selectedGen, selectedTypes,
}) => (
  <div className="pkdx-header-wrap">

    {/* ── Topo vermelho ── */}
    <div className="pkdx-top">
      <div className="pkdx-brand">
        <img src="../../public/game.png" className="pkdx-ball" />
        <span className="pkdx-brand-name">Pokédex Global</span>
      </div>

      {/* Geração — scroll horizontal */}
      <div className="pkdx-gen-strip">
        {GENS.map((g) => (
          <button
            key={g}
            className={`gen-chip ${selectedGen === g ? 'active' : ''}`}
            onClick={() => onGenChange(g)}
          >
            <GiSpartanHelmet style={{ marginRight: '6px' }} />
            Gen {g}
          </button>
        ))}
      </div>

      {/* Barra de busca */}
      <div className="pkdx-search-row">
        <IonSearchbar
          debounce={400}
          placeholder="Nome ou número do Pokémon..."
          onIonInput={(e) => onSearch(e.detail.value ?? '')}
          style={{
            '--border-radius': '12px 12px 0 0',
            '--background': '#fff',
            '--box-shadow': 'none',
            '--padding-start': '12px',
          }}
        />
      </div>
    </div>

    {/* ── Faixa branca de tipos ── */}
    <div className="pkdx-type-strip">
      {TYPES.map((t) => {
        const active = selectedTypes.includes(t.name);
        return (
          <button
            key={t.name}
            className="type-chip"
            onClick={() => onTypeToggle(t.name)}
            style={{
              background: active ? t.color : '#F0F0F0',
              color: active ? t.text : '#555',
            }}
          >
            {t.icon}
            {t.name}
          </button>
        );
      })}
    </div>
  </div>
);

export default PokemonHeader;
